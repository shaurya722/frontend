...
export default function ReportsExport({
  ...
  const generateReport = (format: 'excel' | 'word' | 'pdf') => {
    ...
    // Mock download
    alert(
      `${selectedReportType} report would be downloaded as ${format.toUpperCase()} file`,
    )
  }
  ...
  const getSitesByMunicipality = (municipality: string) => {
    return safeSites.filter(
      (site) =>
        site.municipality?.name === municipality && site.status === 'Active',
    ).length
  }
                    ...
                    safeMunicipalities
                      ...
                      .map((municipality) => (
                        ...
                        >
                          ...
                          >
                            {municipality.name} (
                            {getSitesByMunicipality(municipality.name)} sites)
                          </Label>
                        </div>
                      ))
                  )}
                ...
                <Table>
                  ...
                  <TableBody>
                    {safeMunicipalities
                      ...
                      .map((municipality) => {
                        const municipalitySites = getSitesByMunicipality(
                          municipality.name,
                        )
                        const programsServed = new Set(
                          safeSites
                            .filter(
                              (s) =>
                                s.municipality?.name === municipality.name &&
                                s.status === 'Active',
                            )
                            .flatMap((s) => {
                      ...
                      .map((site) => {
                        ...
                        return (
                          <TableRow key={site.id}>
                            ...
                            <TableCell>
                              {site.municipality?.name || 'Not specified'}
                            </TableCell>
                            <TableCell>
                              <div className='flex flex-wrap gap-1'>
                                {Array.isArray(site.programs) &&
                    ...
                    {safeSites
                      ...
                      .map((site) => (
                        <TableRow key={site.id}>
                          ...
                          <TableCell>
                            {site.active_dates || 'Not specified'}
                          </TableCell>
                          <TableCell>
                            {site.municipality?.name || 'Not specified'}
                          </TableCell>
            ...
            selectedReportType !== 'historical-tracking' && (
              <div className='text-center py-8 text-gray-500'>
                ...
                <p>
                  Report preview for{' '}
                  {
                    reportTypes.find((t) => t.value === selectedReportType)
                      ?.label
                  }
                </p>
                <p className='text-sm'>
                  Configure filters and export to view full report
                </p>
              </div>
            )}
        </CardContent>
