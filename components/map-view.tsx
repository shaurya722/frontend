...
export default function MapView({ sites, municipalities }: MapViewProps) {
  ...
  const [mapLayers, setMapLayers] = useState<MapLayer[]>([
    ...
    { id: "municipalities", name: "Municipality Boundaries", visible: true, color: "#10b981" },
    { id: "population", name: "Population Density", visible: false, color: "#f59e0b" },
  ])
  ...
  const handleLegendClick = (operatorType: string) => {
    const sitesOfType = filteredSites.filter(s => s.operator_type === operatorType)
  ...
  const isSiteActiveInYear = (site: CollectionSite, year: number): boolean => {
    ...
    try {
      ...
      console.warn("[MapView] Error parsing active_dates for site:", site.name, site.active_dates, e)
      return true
    }
  ...
  const filteredSites = safeSites.filter((site) => {
    ...
    const matchesSearch = !searchLocation || 
      ...
      site.municipality?.name?.toLowerCase().includes(searchLocation.toLowerCase()) ||
      site.operator_type?.toLowerCase().includes(searchLocation.toLowerCase())
    
    const matchesStatus = mapFilters.status === "all" || site.status === mapFilters.status
    const sitePrograms = site.programs || []
    const matchesProgram =
      mapFilters.program === "all" || (Array.isArray(sitePrograms) && sitePrograms.includes(mapFilters.program))
    const matchesMunicipality = mapFilters.municipality === "all" || site.municipality?.name === mapFilters.municipality
    const operatorType = site.operator_type || ""
    const matchesOperatorType = mapFilters.operatorType === "all" || operatorType === mapFilters.operatorType

    const matchesSiteType = mapFilters.siteType === "all" || site.site_type === mapFilters.siteType

    return matchesPerformancePeriod && matchesSearch && matchesStatus && matchesProgram && matchesMunicipality && matchesOperatorType && matchesSiteType
  })

  // Debug: Log filtered results
  useEffect(() => {
    if (safeSites.length > 0) {
      console.log("[MapView] Filtered sites count:", filteredSites.length, "out of", safeSites.length)
      if (filteredSites.length === 0 && safeSites.length > 0) {
        console.warn("[MapView] All sites filtered out! Checking first site:", {
          ...
          matchesPerformancePeriod: isSiteActiveInYear(safeSites[0], currentYear),
          currentYear
        })
      }
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-4 pb-4">
          <div className="flex flex-col gap-4">
            {/* Filters Row */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-10 gap-3 items-end">
              {/* Layer Control */}
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground invisible">Layers</Label>
                <Dialog>
                  ...
                  <DialogContent className="w-[95vw] max-w-md">
                    <DialogHeader>
                      ...
                      <DialogDescription>Toggle map layers on/off</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                      {mapLayers.map((layer) => (
                        <div key={layer.id} className="flex items-center justify-between">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => handleLayerToggle(layer.id)}>
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value) => setMapFilters({ ...mapFilters, status: value })}
                >
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value) => setMapFilters({ ...mapFilters, program: value })}
                >
                  <SelectTrigger className="h-9">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value) => setMapFilters({ ...mapFilters, municipality: value })}
                >
                  <SelectTrigger className="h-9">
              ...
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">Operator Type</Label>
                <Select
                  value={mapFilters.operatorType}
                  onValueChange={(value) => setMapFilters({ ...mapFilters, operatorType: value })}
                >
                  <SelectTrigger className="h-9">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value) => setMapFilters({ ...mapFilters, siteType: value })}
                >
                  <SelectTrigger className="h-9">
              ...
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">Performance Period</Label>
                <Select
                  value={mapFilters.performancePeriod}
                  onValueChange={(value) => setMapFilters({ ...mapFilters, performancePeriod: value })}
                >
                  <SelectTrigger className="h-9">
              ...
              <div className="space-y-1">
                ...
                <Button
                  ...
                  onClick={() =>
                    setMapFilters({ status: "all", program: "all", municipality: "all", operatorType: "all", siteType: "all", performancePeriod: currentYear.toString() })
                  }
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-6">
        ...
        <div className="lg:col-span-3 order-2 lg:order-1">
          <Card>
            <CardContent className="p-0 relative" style={{ zIndex: 1, isolation: 'isolate' }}>
              <Suspense
                fallback={
                  ...
                  <Card className="w-80">
                    <CardContent className="pt-6">
                      <div className="text-center space-y-4">
                        ...
                        <div>
                          <h3 className="font-semibold text-lg mb-2">Loading Map Data</h3>
                          <p className="text-sm text-gray-600">
                            Adding {filteredSites.filter(s => s.latitude && s.longitude).length} collection sites to the map...
                          </p>
                        </div>
              ...
              <div className="absolute bottom-4 left-4 bg-white p-2 rounded-lg shadow-lg z-[1000] max-w-xs text-xs">
                <div className="text-xs font-semibold mb-2">Legend – Operator Types</div>
                <div className="space-y-1">
                  {uniqueOperatorTypes.length > 0 ? (
                    uniqueOperatorTypes.map((operatorType) => (
                      ...
                      >
                        <div className={`w-2 h-2 rounded-full`} style={{ backgroundColor: getOperatorTypeColor(operatorType) }}></div>
                        <span className="truncate">{operatorType}</span>
                      </div>
                    ))
                  ) : (
                    <div className="text-xs text-gray-500">No operator types available</div>
                  )}
                </div>
                ...
                <div className="space-y-0.5 text-xs">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>Active: {filteredSites.filter(s => s.status === "Active").length}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 bg-yellow-500 rounded-full"></div>
                    <span>Scheduled: {filteredSites.filter(s => s.status === "Scheduled").length}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 bg-red-500 rounded-full"></div>
                    <span>Inactive: {filteredSites.filter(s => s.status === "Inactive").length}</span>
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <div className="space-y-4 order-1 lg:order-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{filteredSites.length} sites visible</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 max-h-96 overflow-y-auto">
              {filteredSites.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  <p>No sites match current filters</p>
                  <p className="text-sm">Adjust map filters to see more sites</p>
                </div>
              ) : (
                filteredSites.slice(0, 8).map((site) => (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <Badge className={`${getStatusColor(site.status || "")} text-white border-0`}>
                        {site.status}
                      </Badge>
                    </div>
                    <div className="flex flex-wrap gap-1">
                      <Badge className="text-white border-0 text-xs" style={{ backgroundColor: getOperatorTypeColor(site.operator_type || "") }}>
                        {site.operator_type}
                      </Badge>
                      ...
                      {Array.isArray(site.programs) && site.programs.length > 2 && (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{safeMunicipalities.length} communities tracked</CardDescription>
            </CardHeader>
              ...
              {safeMunicipalities.slice(0, 6).map((municipality) => (
                <div key={municipality.id} className="border rounded-lg p-3">
                  ...
                  <div className="text-xs text-gray-600">
                    <div>Population: {(municipality.population || 0).toLocaleString()}</div>
                    <div>Sites: {safeSites.filter((s) => s.municipality?.name === municipality.name).length}</div>
                  </div>
                </div>
              ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge className={`${getStatusColor(selectedSite.status || "")} text-white border-0`}>
                      {selectedSite.status}
                    </Badge>
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge className="text-white border-0" style={{ backgroundColor: getOperatorTypeColor(selectedSite.operator_type || "") }}>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm">{selectedSite.municipality?.name || "Unknown"}</p>
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium">Population Served</Label>
                  <p className="text-sm">{(selectedSite.population_served || 0).toLocaleString()}</p>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600">
                    {selectedSite.created_at ? new Date(selectedSite.created_at).toLocaleDateString() : "N/A"}
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {(!selectedSite.programs || selectedSite.programs.length === 0) && (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-2xl font-bold">{filteredSites.filter(s => s.operator_type === selectedLegendType).length}</p>
                </div>
                <div>
                  <Label className="text-sm font-medium">On Map</Label>
                  <p className="text-2xl font-bold text-blue-600">
                    {filteredSites.filter(s => s.operator_type === selectedLegendType && s.latitude && s.longitude).length}
                  </p>
                </div>
              </div>

              <div>
                <Label className="text-sm font-medium mb-2 block">Sites List</Label>
                    ...
                    .map((site) => (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <h4 className="font-medium text-sm">{site.name}</h4>
                            <p className="text-xs text-gray-600">{site.municipality?.name || "Unknown"}</p>
                          </div>
                          <Badge className={`${getStatusColor(site.status || "")} text-white border-0`}>
                            {site.status}
                          </Badge>
                ...
                {filteredSites.filter(s => s.operator_type === selectedLegendType).length > 10 && (
                  <p className="text-sm text-gray-500 text-center mt-2">
                    ... and {filteredSites.filter(s => s.operator_type === selectedLegendType).length - 10} more sites
                  </p>
                )}
              </div>

              <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded-lg">
                <strong>Tip:</strong> Click on any site above or on the map markers to see detailed information.
              </div>
            </div>
          )}
        </DialogContent>
