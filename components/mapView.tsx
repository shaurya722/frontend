...
export default function MapView({ sites, municipalities }: MapViewProps) {
  ...
  const filteredSites: CollectionSite[] = useMemo(() => {
    ...
    const result = (safeSites || []).filter((site: CollectionSite) => {
      ...
      // For API data, be more lenient since active_dates might be creation dates
      // For fallback data, use strict filtering
        ...
        <div className='space-y-4 order-1 2xl:order-2'>
            ...
            <Card>
              <CardHeader>
                <CardTitle className='text-lg flex items-center gap-2'>
                  ...
                  Census Year {apiData.census_year.year}
                </CardTitle>
                <CardDescription>
                  Data for {apiData.census_year.year} performance period
                </CardDescription>
              </CardHeader>
            </Card>
          )}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {safeMunicipalities.length} communities tracked {isLoadingData && '(loading...)'}
              </CardDescription>
            </CardHeader>
                ...
                <div key={municipality.id} className='border rounded-lg p-3'>
                  ...
                  <div className='text-xs text-gray-600'>
                    ...
                    <div>
                      ...
                      {
                        safeSites.filter(
                          (s: CollectionSite) => s.municipality?.name === municipality.name,
                        ).length
                      }
                    </div>
                  </div>
