...
export default function ToolADirectServiceOffset() {
  ...
  const calculateBaseRequirement = useCallback(
    (population: number, program: string): number => {
      switch (program) {
        case 'Paint':
  ...
  const calculateNewRequired = useCallback(
    (required: number, percentage: number): number => {
  ...
  const handleApplyAll = useCallback(() => {
    const newResults = communities.map((community) => {
      const required = calculateBaseRequirement(
    ...
    globalPercentage,
    selectedProgram,
    calculateBaseRequirement,
    calculateNewRequired,
  ])
  ...
  const handleEditCommunity = (id: string) => {
    setResults(
      results.map((r) =>
        r.id === id
          ? { ...r, isEditing: true, customPercentage: r.percentageReduction }
          : r,
      ),
    )
  }
  ...
  const handleCancelEdit = (id: string) => {
    setResults(
      results.map((r) =>
        r.id === id
          ? { ...r, isEditing: false, customPercentage: undefined }
          : r,
      ),
    )
  }
  ...
  const handleCustomPercentageChange = (id: string, value: number) => {
    setResults(
      results.map((r) =>
        r.id === id
          ? { ...r, customPercentage: Math.min(100, Math.max(0, value)) }
          : r,
      ),
    )
  }
      ...
      <Card>
        ...
        <CardContent>
          ...
          <Alert className='mt-4'>
            ...
            <AlertDescription>
              <strong>Calculation:</strong> New Required = ceil(Required × (1 -
              %reduction)). Every community requiring at least 1 site will still
              require minimum 1 site (cannot be reduced below 1).
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {totalOriginalRequired > 0
                  ? Math.round((totalReduction / totalOriginalRequired) * 100)
                  : 0}
                %
              </div>
              <p className='text-xs text-muted-foreground'>
        ...
        <CardContent>
          {results.length === 0 ? (
            <div className='text-center py-8 text-muted-foreground'>
              ...
              <p className='text-sm'>
                Set a global percentage and click "Apply All" to calculate
              </p>
            </div>
          ) : (
                      ...
                      <TableRow key={result.id}>
                        ...
                        <TableCell className='text-right'>
                          {result.isEditing ? (
                            ...
                            <Badge
                              variant={
                                result.percentageReduction !== globalPercentage
                                  ? 'default'
                                  : 'secondary'
                              }
                            >
                          ...
                          {result.isEditing ? (
                            <div className='flex items-center justify-center gap-1'>
                              <Button
                                ...
                                size='icon'
                                onClick={() =>
                                  handleSaveCommunityEdit(result.id)
                                }
                              >
                            ...
                            <Button
                              variant='ghost'
                              size='icon'
                              onClick={() => handleEditCommunity(result.id)}
                            >
