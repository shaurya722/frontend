...
export default function ToolBEventApplication() {
  ...
  const { data: eventData, isLoading } = useEventListing(selectedYear, debouncedSearch || undefined, currentPage, pageSize)

  const approveMutation = useMutation({
    mutationFn: async (data: { site_ids: number[], is_event: boolean }) => {
      const response = await fetch('http://127.0.0.1:8000/api/sites/approve-events/', {
        method: 'PUT',
  ...
  useEffect(() => {
    if (eventData) {
      ...
      const transformedData: ShortfallCommunity[] = eventData.results.map((item: ApiResponse) => ({
        ...
        appliedEvents: item.Events.slice(0, item.applied_event).map((e: Event) => e.id),
      }))
  ...
  const handleApplyEvents = async () => {
    ...
    try {
      ...
      const siteIdsUnchecked = selectedCommunity.appliedEvents.filter(id => !selectedEvents.includes(id))

      // Call API for unchecked events with false and selected with true
      await Promise.all([
        siteIdsUnchecked.length > 0 ? approveMutation.mutateAsync({ site_ids: siteIdsUnchecked, is_event: false }) : Promise.resolve(),
        siteIdsSelected.length > 0 ? approveMutation.mutateAsync({ site_ids: siteIdsSelected, is_event: true }) : Promise.resolve(),
      ])
      ...
      setSelectedEvents([])
      setSuccessMessage(`Events updated successfully for ${selectedCommunity.name}`)
    } catch (error: any) {
      ...
      <Dialog open={isEventDialogOpen} onOpenChange={setIsEventDialogOpen}>
        <DialogContent className='max-w-lg'>
          ...
          <div className='space-y-4'>
            ...
            <div className='max-h-64 overflow-y-auto space-y-2'>
              {selectedCommunity?.events.map((event) => {
                ...
                const handleClick = () => handleEventToggle(event.id, !isSelected)
                return (
                  <div
                    key={event.id}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleApplyEvents} disabled={approveMutation.isPending}>
              {approveMutation.isPending ? 'Saving...' : `Save ${selectedEvents.length} Event(s)`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
