...
export default function ToolBEventApplication() {
  ...
  const fetchData = async () => {
    ...
    try {
      console.log(
        '[ToolB] Fetching data for program:',
        selectedProgram,
        'year:',
        selectedYear,
      )
      ...
      const communitiesData: Municipality[] = communitiesResponse.results.map(
        (m) => ({
          id: m.id,
          name: m.name,
      ...
      try {
        ...
        const toolBData = await getToolBData({
        ...
        if (toolBData.data?.communities) {
          const appliedMap = toolBData.data.communities.reduce(
  ...
  const [appliedEventsFromBackend, setAppliedEventsFromBackend] = useState<
    Record<string, string[]>
  >({})

  // Calculate base requirement
  const calculateRequirement = useCallback(
  ...
  const calculateShortfalls = useCallback(() => {
    ...
    communities.forEach((community) => {
      const required = calculateRequirement(
  ...
  const totalRequired = communities.reduce(
    (sum, c) => sum + calculateRequirement(c.population, selectedProgram),
    0,
  )
    ...
    try {
      ...
      if (toolBData.data?.communities) {
        setAppliedEventsFromBackend(
          toolBData.data.communities.reduce(
            (acc: Record<string, string[]>, c: any) => {
              acc[c.id] = c.applied_events || []
              return acc
            },
        ...
        <CardContent className='pt-6'>
          <div className='flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4'>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div className='space-y-1 [&_button]:min-w-[160px] [&_button]:md:min-w-full'>
