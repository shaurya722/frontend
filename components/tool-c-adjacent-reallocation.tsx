...
import { GitBranch, ArrowRight, Info, CheckCircle, Search, ChevronLeft, ChevronRight, ArrowUpDown } from "lucide-react"
...
export default function ToolCAdjacentReallocation() {
    ...
    const [selectedExcessCommunity, setSelectedExcessCommunity] = useState<ExcessCommunity | null>(null)
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (response.data && (response.data.results || response.data.communities)) {
    ...
    const handleReallocate = async () => {
        if (!selectedExcessCommunity || selectedSites.length === 0 || !targetCommunity) return

        try {
            const targetComm = selectedExcessCommunity.adjacentWithShortfalls.find((a) => a.id === targetCommunity)
        ...
        <div className="space-y-6">
            ...
            <Alert>
                ...
                <AlertDescription>
                    ...
                    <ul className="list-disc list-inside mt-1 text-sm">
                        ...
                        <li>Operator type is Retailer, Distributor, Private Depot, Product Care, or Other</li>
                        <li>The destination community directly borders the source community</li>
                    </ul>
                </AlertDescription>
            </Alert>
                ...
                <CardContent className="pt-6">
                    <div className="flex flex-col lg:flex-row gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Select value={selectedProgram} onValueChange={(v) => { setSelectedProgram(v); setPage(1); }}>
                                        ...
                                        <TableRow>
                                            ...
                                            >
                                                <div className="flex items-center justify-end gap-1">
                                                    Eligible Excess
                                                    <ArrowUpDown className="h-3 w-3" />
                                                    <span className="text-xs">{getSortIcon("eligible_excess")}</span>
                                                </div>
                                            </TableHead>
                                            <TableHead 
                                                className="cursor-pointer hover:bg-muted/50 bg-gray-50"
                                            ...
                                            >
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <ArrowUpDown className="h-3 w-3" />
                                                    <span className="text-xs">{getSortIcon("adjacent_shortfalls")}</span>
                                                </div>
                                            </TableHead>
                                            <TableHead className="cursor-pointer hover:bg-muted/50 bg-gray-50">Adjacent Reallocation</TableHead>
                                            <TableHead className="text-center cursor-pointer hover:bg-muted/50 bg-gray-50">Actions</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                            ...
                                            <TableRow key={community.id}>
                                                    ...
                                                    {community.adjacentWithShortfalls.length === 0 ? (
                                                        ...
                                                        <div className="flex flex-wrap gap-1">
                                                            {community.adjacentWithShortfalls.map((adj) => (
                                                                <Badge key={adj.id} variant="destructive" className="text-xs">
                                                ...
                                                <TableCell>
                                                    {community.adjacentWithShortfalls.length === 0 ? (
                                                        ...
                                                        <div className="flex flex-wrap gap-1">
                                                            {community.adjacentWithShortfalls.filter(adj => adj.total_reallocated > 0).map((adj) => (
                                                                <Badge key={adj.id} variant="secondary" className="text-xs bg-green-100 text-green-700">
                                                ...
                                                <TableCell className="text-center">
                                                    <Button
                                                        size="sm"
                                                        onClick={() => handleOpenReallocationDialog(community)}
                                                        disabled={community.adjacentWithShortfalls.length === 0}
                                                    >
                                                        Reallocate
                                                    </Button>
                                                </TableCell>
            ...
            <Dialog open={isReallocationDialogOpen} onOpenChange={setIsReallocationDialogOpen}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>Reallocate Sites from {selectedExcessCommunity?.name}</DialogTitle>
                        <DialogDescription>Select sites to reallocate and choose a destination community.</DialogDescription>
                    </DialogHeader>

                    <div className="space-y-4">
                        {/* Target Community Selection */}
                        <div className="space-y-2">
                            <label className="text-sm font-medium">Destination Community</label>
                            <Select value={targetCommunity} onValueChange={setTargetCommunity}>
                                ...
                                <SelectContent>
                                    {selectedExcessCommunity?.adjacentWithShortfalls.map((adj) => (
                        ...
                        {/* Site Selection */}
                        <div className="space-y-2">
                            <label className="text-sm font-medium">Select Sites to Reallocate</label>
                        ...
                        {selectedSites.length > 0 && targetCommunity && (
                            <Alert>
                                ...
                                <AlertDescription>
                                    ...
                                    <strong>
                                        {selectedExcessCommunity?.adjacentWithShortfalls.find((a) => a.id === targetCommunity)?.name}
                                    </strong>
                                    .
                                </AlertDescription>
                            </Alert>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleReallocate} disabled={selectedSites.length === 0 || !targetCommunity}>
